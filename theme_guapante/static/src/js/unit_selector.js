...
publicWidget.registry.GuapanteUnitSelector = publicWidget.Widget.extend({
    ...
    _onAddToCart: async function (ev) {
        ...
        let packagingId = 0;
        console.log("GUAPANTE-DEBUG: qtyInput=", qtyInput, "typeof=", typeof qtyInput);
        ...
        try {
            ...
            if (data.error) {
                console.error("GUAPANTE-DEBUG: ❌ SERVER ERROR:", data.error.data ? data.error.data.message : data.error.message);
            }
            ...
            console.error("GUAPANTE-DEBUG: ❌ AJAX ERROR:", error.status, error.statusText, error.responseText ? error.responseText.substring(0, 300) : "");
